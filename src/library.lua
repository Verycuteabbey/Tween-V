...
local function __color3(A: Color3, B: Color3, alpha: number): Color3
	...
	return newColor3(__number(R1, R2, alpha), __number(G1, G2, alpha), __number(B1, B2, alpha))
end
local function __colorSequenceKeypoint(
	A: ColorSequenceKeypoint,
	B: ColorSequenceKeypoint,
	alpha: number
): ColorSequenceKeypoint
	...
	local color = newColor3(__number(R1, R2, alpha), __number(G1, G2, alpha), __number(B1, B2, alpha))
...
local function __numberSequenceKeypoint(
	...
	return newNumberSequenceKeypoint(__number(T1, T2, alpha), __number(V1, V2, alpha), __number(E1, E2, alpha))
end
local function __ray(A: Ray, B: Ray, alpha: number): Ray
	local D1, D2 = A.Direction, B.Direction
	local O1, O2 = A.Origin, B.Origin

	local V1 = newVector3(__number(O1.X, O2.X, alpha), __number(O1.Y, O2.Y, alpha), __number(O1.Z, O2.Z, alpha))
	local V2 = newVector3(__number(D1.X, D2.X, alpha), __number(D1.Y, D2.Y, alpha), __number(D1.Z, D2.Z, alpha))
