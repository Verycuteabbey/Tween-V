...
--// defines
local defer, delay, spawn, wait = task.defer, task.delay, task.spawn, task.wait
...
function controller:Create(
    ...
    function funcs:Update(deltaTime: number)
        ...
        for K, V in pairs(target) do
            local variant = library:Lerp(easeOption, info.current[K], V, status.nowTime / duration)
            instance[K] = variant
        end
...
defer(function()
    while true do
        for position, tween: table in pairs(active) do
            ...
            if not status.started or status.ended then
                active[position] = nil
            end
        end
